...
bool Problem::IsGoodStepInLM() {
    ...
    //a = ((J^T*W*residual_)^T*delta_x_)/((currentChi_ - tempChi)/2+2*(J^T*W*residual_)^T*delta_x_)

    double fc = b_.transpose()*delta_x_;
    double a = fc/(((tempChi-currentChi_)/2.)+2*fc);
    RollbackStates();
    delta_x_ *= a;
    UpdateStates();    
    ...
    {
        currentLambda_ = std::max(currentLambda_/(1+a), 1e-7);
        currentChi_ = tempChi;
        return true;
    } else {
        currentLambda_ = currentLambda_+abs(currentChi_ - tempChi)/(2.*a);
        return false;
    }
}
